...
export const useCustomBlockStore = defineStore("customBlocks", () => {
  const blocks = ref({});
  function update(updatedTheme) {
    blocks.value = updatedTheme;
  }
  const blocksArray = computed(() => {
    const themeArray: Block[] = Object.entries(blocks.value).map((
      [key, value],
    ) => ({ key, value: value as JumbotronInterface | MediaSectionInterface }));
    return themeArray;
  });
